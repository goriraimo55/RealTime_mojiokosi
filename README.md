...
設定タブ最下部のアプリ版が `2026.08.15.2` 以降であることも確認してください。版表示がない、または古い場合は、開いている `index.html` 自体が最新版ではありません。

#### 議事録が途中で切れる場合

LM StudioのログやAPI応答で `finish_reason: "length"` となる場合、モデルが最大出力トークン数に達しています。最新版のアプリは1回あたり最大4,096トークンを要求し、`length`で終了した場合は切れた箇所から最大3回まで自動的に続きを生成します。

それでも切れる場合は、LM Studioでモデルをロードする画面の設定から **Context Length** を増やしてください。入力する文字起こしと出力する議事録の両方がこの枠を使うため、まず16,384、余裕があれば32,768を目安にします。値を増やすとRAM / VRAM使用量も増えるため、モデルを再ロードできない場合は一段階下げてください。LM Studioのバージョンによって **Max Tokens / Prediction Length** の設定が表示される場合は4,096以上にします。
